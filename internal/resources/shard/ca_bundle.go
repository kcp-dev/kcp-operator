...
func MergedCABundleSecretReconciler(ctx context.Context, shard *operatorv1alpha1.Shard, kubeClient client.Client) k8creconciling.NamedSecretReconcilerFactory {
	return func() (string, k8creconciling.SecretReconciler) {
		...
		return secretName, func(secret *corev1.Secret) (*corev1.Secret, error) {
			...
			if shard.Spec.CABundleSecretRef != nil {
				...
				err := kubeClient.Get(ctx, types.NamespacedName{
					Name:      shard.Spec.CABundleSecretRef.Name,
					Namespace: shard.Namespace,
				}, userCABundleSecret)
				if err != nil {
					return nil, fmt.Errorf("failed to get user CA bundle secret %s: %w", shard.Spec.CABundleSecretRef.Name, err)
				}

				var exists bool
				userCABundle, exists = userCABundleSecret.Data["tls.crt"]
				if !exists {
					return nil, fmt.Errorf("user CA bundle secret %s missing tls.crt", shard.Spec.CABundleSecretRef.Name)
				}
			}
