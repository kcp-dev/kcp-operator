...
// +kubebuilder:rbac:groups=operator.kcp.io,resources=bundles,verbs=get;list;watch;update;patch
...
func (r *BundleReconciler) reconcile(ctx context.Context, bundle *operatorv1alpha1.Bundle) error {
	...
	var requiredObjects []operatorv1alpha1.BundleObject
...
// createBundleObjects creates ConfigMap and Secret objects containing the bundle data
func (r *BundleReconciler) createBundleObjects(ctx context.Context, bundle *operatorv1alpha1.Bundle, requiredObjects []operatorv1alpha1.BundleObject) error {
	...
	objects := make([]ctrlruntimeclient.Object, len(requiredObjects))
	for _, obj := range requiredObjects {
		resource := newResourceForGVR(obj.GVR.Resource)
		if resource == nil {
			continue
		...
		if err := r.Get(ctx, ctrlruntimeclient.ObjectKey{
			...
			logger.V(1).Info("failed to get object for bundle", "object", obj.String(), "error", err)
			continue
		}
	...
	// Create or update the bundle Secret
	...
	if err := r.Get(ctx, ctrlruntimeclient.ObjectKey{
		Name:      bundle.Name,
		Namespace: bundle.Namespace,
	}, bundleSecret); err != nil {
		if apierrors.IsNotFound(err) {
			// Create new Secret
			bundleSecret.Data = bundleData
			bundleSecret.OwnerReferences = []metav1.OwnerReference{
				*metav1.NewControllerRef(bundle, operatorv1alpha1.SchemeGroupVersion.WithKind("Bundle")),
			}
			if err := r.Create(ctx, bundleSecret); err != nil {
				return fmt.Errorf("failed to create bundle secret: %w", err)
			}
			logger.Info("created bundle secret", "secret", bundleSecret.Name)
		} else {
		...
		bundleSecret.Data = bundleData
		if err := r.Update(ctx, bundleSecret); err != nil {
			return fmt.Errorf("failed to update bundle secret: %w", err)
		}
		logger.V(1).Info("updated bundle secret", "secret", bundleSecret.Name)
	}
...
// Similar to BundleToSecretData in the old implementation
...
func (r *BundleReconciler) updateStatus(ctx context.Context, bundle *operatorv1alpha1.Bundle, oldStatus *operatorv1alpha1.BundleStatus, conditions []metav1.Condition, errs []error) error {
	...
	case readyCond != nil && readyCond.Status == metav1.ConditionTrue:
		bundle.Status.Phase = operatorv1alpha1.BundlePhaseReady

	case bundle.DeletionTimestamp != nil:
		bundle.Status.Phase = operatorv1alpha1.BundlePhaseDeleting

	case bundle.Status.Phase == "":
		bundle.Status.Phase = operatorv1alpha1.BundlePhaseProvisioning
	}
