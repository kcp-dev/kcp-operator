...
func GetBundleObjectsForShard(shard *operatorv1alpha1.Shard, rootShardName string) []operatorv1alpha1.BundleObject {
	...
	return []operatorv1alpha1.BundleObject{
		...
		// Merged CA bundle if configured
		{GVR: secretGVR, Name: fmt.Sprintf("%s-merged-ca-bundle", shardName), Namespace: namespace},

		// Deployment
		{GVR: deploymentGVR, Name: fmt.Sprintf("%s-shard-kcp", shardName), Namespace: namespace},

		// Service
		{GVR: serviceGVR, Name: fmt.Sprintf("%s-shard-kcp", shardName), Namespace: namespace},
	}
}
