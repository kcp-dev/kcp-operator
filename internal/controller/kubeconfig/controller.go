...
func (r *KubeconfigReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	kcCopy := kc.DeepCopy()
	kcCopy.Status.TargetName = kc.GetTargetName()
...
func (r *KubeconfigReconciler) mapRootShardToKubeconfigs(ctx context.Context, obj ctrlruntimeclient.Object) []ctrl.Request {
	...
	if err := r.List(ctx, &kubeconfigs); err != nil {
		logger.V(4).Error(err, "Failed to list Kubeconfigs")
		return []ctrl.Request{}
	}

	var requests []ctrl.Request
	for _, kc := range kubeconfigs.Items {
		if kc.Spec.Target.RootShardRef != nil && kc.Spec.Target.RootShardRef.Name == rootShard.Name {
	...
	logger.V(4).Info("Mapped RootShard to Kubeconfigs", "requests", requests)
	return requests
}
...
func (r *KubeconfigReconciler) mapShardToKubeconfigs(ctx context.Context, obj ctrlruntimeclient.Object) []ctrl.Request {
	...
	var requests []ctrl.Request
	for _, kc := range kubeconfigs.Items {
		if kc.Spec.Target.ShardRef != nil && kc.Spec.Target.ShardRef.Name == shard.Name {
			requests = append(requests, ctrl.Request{
				NamespacedName: types.NamespacedName{
					Name:      kc.Name,
					Namespace: kc.Namespace,
	...
	logger.V(4).Info("Mapped Shard to Kubeconfigs", "requests", requests)
	return requests
}
...
func (r *KubeconfigReconciler) mapFrontProxyToKubeconfigs(ctx context.Context, obj ctrlruntimeclient.Object) []ctrl.Request {
	...
	var requests []ctrl.Request
	for _, kc := range kubeconfigs.Items {
		if kc.Spec.Target.FrontProxyRef != nil && kc.Spec.Target.FrontProxyRef.Name == frontProxy.Name {
			requests = append(requests, ctrl.Request{
				NamespacedName: types.NamespacedName{
					Name:      kc.Name,
					Namespace: kc.Namespace,
	...
	logger.V(4).Info("Mapped FrontProxy to Kubeconfigs", "requests", requests)
	return requests
}
